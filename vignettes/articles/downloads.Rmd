...
description: "Explore CRAN download trends for rOpenGov packages."
---
...
# Search in CRAN
cran_all <- tools::CRAN_package_db() |>
  as_tibble()

# URL or Bug
cranpaks <- cran_all |>
  filter(
    str_detect(tolower(URL), "ropengov") |
      str_detect(tolower(BugReports), "ropengov")
  ) |>
  pull(Package)


# Direct search
crandb <- unique(cran_all$Package)
...
# Packages in CRAN
mypaks <- c(mypaks[mypaks %in% crandb], cranpaks) |>
  unique() |>
  sort()

# fmi package in CRAN is not the one we have in here
mypaks <- mypaks[mypaks != "fmi"]
...
```{r}
#| echo: false
#| fig-alt: Monthly downloads of packages hosted by rOpenGov
tot <- dwn_m |>
  group_by(m) |>
...
#| echo: false
#| fig-asp: 1.5
#| fig-alt: Weekly downloads of packages hosted by rOpenGov

# Not this week
thisw <- lubridate::ceiling_date(Sys.Date(), "week")
...
# Combine summaries and assign display names.
end <-
  fechas |>
  left_join(avg_daily) |>
  left_join(last_day) |>
...
Downloads and package metadata for [rOpenGov](https://ropengov.org/) packages
from [Posit Package Manager](https://packagemanager.posit.co/client/#/).
...
names(res_all) <- c(
  "package", "Latest version", "Version on CRAN", "Package size",
