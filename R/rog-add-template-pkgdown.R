...
rog_add_template_pkgdown <- function() {
  ...
  # Prefer `_pkgdown.yml` in the project root, then fall back to
  ...
  if (file_exist) {
    ...
    params <- newfile$template$params
    if (
      length(params) == 0 ||
        (length(params) == 1 && "bootswatch" %in% names(params))
    ) {
      newfile <- modifyList(
        newfile,
  ...
  cli::cli_inform(c(
    "v" = "Added {.pkg rogtemplate} to {.file {path}}."
  ))
